...
import { Loader2, Search, ExternalLink, Package, Truck, Calendar, User, Building2, ArrowRight } from "lucide-react"
...
const BuyerOrders = () => {
  ...
  const filteredOrders = orders.filter(
    (order) =>
      ...
      order.trackingInfo.status?.toLowerCase().includes(searchTerm.toLowerCase()),
  )
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-8">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <Button asChild className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white shadow-sm hover:shadow-md transition-all duration-300">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {filteredOrders.map((order) => (
              <Card key={order._id} className="bg-white/80 backdrop-blur-sm border-none shadow-lg rounded-2xl overflow-hidden">
                <CardHeader className="border-b border-slate-100 pb-4">
                  <CardTitle className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-lg font-semibold text-slate-900">Order #{order._id.slice(-6)}</span>
                        <p className="text-sm text-slate-500">Placed on {formatDate(order.createdAt)}</p>
                      </div>
                    </div>
                    <span className={`px-4 py-1.5 rounded-full text-sm font-medium ${getStatusBadgeClass(order.trackingInfo.status)}`}>
                      {order.trackingInfo.status.charAt(0).toUpperCase() + order.trackingInfo.status.slice(1)}
                    </span>
                  </CardTitle>
                </CardHeader>
                      ...
                      item.product && (
                        <div key={index} className="flex gap-4 p-4 bg-slate-50 rounded-xl">
                          ...
                          <div className="flex-1">
                            ...
                            <Button variant="link" className="p-0 h-auto text-sm text-blue-600 hover:text-blue-700" asChild>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 bg-slate-50 rounded-xl">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h3 className="text-sm font-medium text-slate-700">Seller Information</h3>
                        </div>
                        <p className="text-sm text-slate-900 font-medium">{order.exporter?.name || "Unknown"}</p>
                        <p className="text-sm text-slate-600">{order.exporter?.email}</p>
                      </div>
                      ...
                      {order.shipper && (
                        <div className="p-4 bg-slate-50 rounded-xl">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <h3 className="text-sm font-medium text-slate-700">Shipping Information</h3>
                          </div>
                          <p className="text-sm text-slate-900 font-medium">{order.shipper?.name}</p>
                          <p className="text-sm text-slate-600">{order.shipper?.email}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {order.trackingInfo.status === "Delivered" && (
                    <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-xl">
                      <p className="text-green-800 text-sm flex items-center gap-2">
                        ...
                        This order has been successfully delivered. Thank you for your purchase!
                      </p>
                    </div>
                  )}
                  ...
                  {order.trackingInfo.status === "Cancelled" && (
                    <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                      <p className="text-red-800 text-sm flex items-center gap-2">
                        ...
                        This order has been cancelled. Please contact the seller for more information.
                      </p>
                    </div>
                  )}
